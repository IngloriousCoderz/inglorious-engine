...
The best documentation to me is one by example. So here's a few examples of games!
...
The static part includes the definition of each `type` of actors we want to show on the screen. For convenience we provide a default type called `character` that shows its position and orientation.
