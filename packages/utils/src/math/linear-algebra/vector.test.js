...
test("it should change the angle of a vector", () => {
  ...
  const expectedResult = [6.123233995736766e-17, 0, 1] // close to v(0, 0, 1)
...
test("it should create a unit vector oriented on the Z-axis", () => {
  expect(unit(pi() / 2)).toStrictEqual([6.123233995736766e-17, 0, 1]) // close to v(0, 0, 1)
})

test("it should create a vector from a list of coordinates", () => {
  expect(v(1, 2, 3)).toStrictEqual(v(1, 2, 3))
...
test("it should create a vector with a __isVector__ property", () => {
  const vector = v(1, 2, 3)
  expect(vector).toStrictEqual(v(1, 2, 3))
