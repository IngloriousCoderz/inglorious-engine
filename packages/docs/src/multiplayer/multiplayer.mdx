...
To start the example server, navigate to the `packages/server` directory of the `inglorious-engine` monorepo and run the following command. The server will automatically use the `server.js` file for this example.

```sh
pnpm dev ../../docs/src/multiplayer/server.js
```
