...
if (collidesWith(mario, fireFlower)) {
    notify("morph", {
        ...
        type: [ // The new array of decorators and handlers
            enableBaseMario(),
            enableSuperMario(),
            enableFireMario(),
...
When the engine processes this event, it will replace the existing definition for the `mario` type with the new one. All existing and future instances of `type: 'mario'` will now use the new set of behaviors, including the abilities from `enableFireMario()`.
