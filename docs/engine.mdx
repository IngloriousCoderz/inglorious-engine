...
To animate stuff we write event handlers for events that are triggered by our instances or by the game itself. The `update` event, for example, is a special core event that is triggered at every cycle of the game loop.
...
Instead of relying on complex class inheritance hierarchies, we use **function composition**. In this example, all the code necessary to draw the character is encapsulated in a `character()` behavior.
...
A key advantage of building games in JavaScript is the language's inherent flexibility. Unlike in many statically-typed languages, JavaScript objects are dynamic. You can add, modify, or remove properties from any `instance` at runtime, without needing to pre-define them in its `type`.

This is incredibly useful in game development. For instance, if a character picks up a temporary power-up, you can simply attach a new property to the character's instance to manage its state.
...
```javascript
// In a 'collision' event handler for the player instance...
instance.powerUp = { type: 'speed', duration: 10 }; // 10 seconds

// Later, in the 'game:update' event handler for the player...
if (instance.powerUp?.type === 'speed') {
    // This function would increase the player's speed
    // (Remember, Immer handles immutability for you!)
    instance.velocity[0] *= 1.5;
    instance.velocity[1] *= 1.5;

    instance.powerUp.duration -= dt;

    if (instance.powerUp.duration <= 0) {
        // Power-up has expired, remove the property.
        delete instance.powerUp;
    }
}
```
...
While adding properties to individual instances at runtime is powerful for temporary state changes, sometimes you need to fundamentally alter the behavior of all instances of a certain type. The engine supports this through the `morph` event.
...
Let's revisit the classic Super Mario example. When Mario picks up a power-up, his abilities change. Instead of managing boolean flags on the instance (`canBreakBricks`, `canShootFire`), we can dynamically change the `mario` type itself.
...
if (collidesWith(mario, fireFlower)) {
    notify("morph", {
        ...
        type: [ // The new array of behaviors and handlers
            ...
            superMario(),
            fireMario(),
            // ... other base decorators like controls, physics etc.
        ],
    });
...
When the engine processes this event, it will replace the existing definition for the `mario` type with the new one. All existing and future instances of `type: 'mario'` will now use the new set of behaviors, including the abilities from `fireMario()`.

This pattern keeps instance objects clean and centralizes behavior logic within type definitions, making your game architecture more robust and easier to reason about. You can see a complete implementation of this pattern in the Mario Power-Ups recipe.
...
This immutable approach provides a major performance benefit, especially for rendering. To determine if a UI component needs to be redrawn, you can perform a simple and highly efficient reference equality check (`prevState.player === nextState.player`).
