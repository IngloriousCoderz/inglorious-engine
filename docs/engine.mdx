...
While adding properties to individual instances at runtime is powerful for temporary state changes, sometimes you need to fundamentally alter the behavior of all instances of a certain type. The engine supports this through the `type:change` event.
...
Here's how you might dispatch a `type:change` event upon collision with a Fire Flower:
...
if (collidesWith(mario, fireFlower)) {
    ...
    notify({ id: "instance:remove", payload: fireFlower.id });
}
```
