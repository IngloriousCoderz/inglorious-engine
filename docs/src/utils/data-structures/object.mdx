...
Unlike Mutative, which uses structural sharing via proxies for high performance, this implementation performs a **full deep clone** of the base state on every call using `deserialize(serialize())`.
...
```javascript
import { create } from '@inglorious/utils/data-structures/object';
...
const incrementAge = create((draft) => {
  draft.user.age += 1;
});
