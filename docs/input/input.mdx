...
If you need mouse support, it's as easy as using the `mouse()` behavior. It automatically detects movement and clicks on any instance in the scene.
...
Want to make both keyboard and gamepad work? Use the `controlsTypes()` and `controlsInstances()` functions then!
