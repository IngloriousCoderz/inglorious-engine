...
This is the key insight. An instance's `type` is not a rigid class, but a dynamic **array of behavior objects**.

When an event occurs for an instance, the engine iterates through its type's behavior array and executes any matching event handlers it finds.
...
```javascript
// A behavior factory that clamps the instance to the screen bounds
...
Because a `type` is just an array, we can change it at any time. By dispatching a `morph` event, you can provide a completely new array of behaviors for an instance type. This is how we achieve the "Decorator Pattern" in a functional way.
...
-   **Dynamic & Pluggable:** You can add or remove behaviors at any point in the game's lifecycle. You could even load new behavior modules from a server and plug them into an existing instance type.
