...
types: {
  mario: [draw, modernControls, jump]
}
```

### 4. Runtime Re-Composition

Because a `type` is just an array, we can change it at any time. By dispatching a `type:change` event, you can provide a completely new array of behaviors for an instance type. This is how we achieve the "Decorator Pattern" in a functional way.
